...
st.write("**Шешім:** Датчиктер арқылы деректерді жинайтын, автоматты суаратын және AI арқылы өсімдік жағдайын талдайтын IoT жүйесі.")

st.markdown("---")

# --- 2. DASHBOARD БӨЛІМІ ---
st.header("📊 Нақты уақыттағы IoT Телеметриясы")
...
# Ылғалдылық графигін түзету: melt қолдану арқылы
df_melted = df.melt(id_vars=["Уақыт"], 
                    value_vars=["Ылғалдылық (%)", "Топырақ ылғалдылығы (%)"], 
                    var_name="Көрсеткіш", 
                    value_name="Мәні")

fig_hum = px.line(df_melted, x="Уақыт", y="Мәні", color="Көрсеткіш", title="Ылғалдылық көрсеткіштері")
st.plotly_chart(fig_hum, use_container_width=True)

st.markdown("---")

# --- 3. AUTOMATION & AI БӨЛІМІ ---
...
with col_act2:
    st.subheader("🧠 AI-Агроном Кеңесі")
    if st.button("AI-дан есеп алу"):
        with st.spinner('Жасанды интеллект деректерді талдауда...'):
            ...
            **AI Генерацияланған Есеп (Дата: {datetime.now().strftime('%d.%m.%Y')}):**
            1. **Қорытынды:** Соңғы сағатта температура {latest_temp}°C-қа дейін көтерілген.
            2. **Ұсыныс:** Ылғалдылық деңгейі {latest_hum}%. Топырақ ылғалдылығын бақылауда ұстаңыз.
